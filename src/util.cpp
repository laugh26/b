...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            if(ptrCategory->count(string("waggox"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Waggox
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Waggox
    // Mac: ~/Library/Application Support/Waggox
    // Unix: ~/.waggox
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Waggox";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Waggox";
#else
    // Unix
    return pathRet / ".waggox";
#endif
#endif
}
