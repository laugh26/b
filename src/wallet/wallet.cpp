            ...
            while (true)
            {
                ...
                if (!SelectCoins(nValueToSelect, setCoins, nValueIn, coinControl, nCoinType, fUseInstaPay))
                {
                    if (nCoinType == ONLY_NOT100000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 WAGGOX.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT100000IFMN) {
                        strFailReason = _("Unable to locate enough PrivatePay non-denominated funds for this transaction that are not equal 1000 WAGGOX.");
                    ...
                    if (fUseInstaPay) {
                        if (nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTAPAY_MAX_VALUE)*COIN) {
                            strFailReason += " " + strprintf(_("InstaPay doesn't support sending values that high yet. Transactions are currently limited to %1 WAGGOX."), sporkManager.GetSporkValue(SPORK_5_INSTAPAY_MAX_VALUE));
                        } else {
